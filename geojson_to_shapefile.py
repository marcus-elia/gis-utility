...
def main():
    ...
    parser.add_argument("-o", "--output-shapefile-path", required=True, help="Path to output shapefile file.")
    ...
    gdf.to_file(args.output_shapefile_path)

    # Note: this is not needed because you can give a .shp.zip filepath to geopandas.to_file().
    if args.zip:
        base_filepath = args.output_shapefile_path.split('.')[-1]
